        ...
        if test_dir.exists():
            for file_path in test_dir.rglob("*.py"):
                if file_path.is_file():
                    ...
                    if "smoke" in str(file_path) or "imports" in str(file_path) or "backends" in str(file_path):
                        add_file_to_snapshot(outfile, file_path, original_cwd)
...
def add_file_to_snapshot(outfile, file_path, project_root):
    ...
    if file_path.exists() and file_path.is_file():
        try:
            ...
            if not any(excl in rel_path_str for excl in ["/legacy/", "/__pycache__/", "/snapshot/", "/gomobile-matsuri-src/"]):
