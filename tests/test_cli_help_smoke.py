...
def test_all_commands_show_help():
    ...
    for line in lines:
        ...
        if in_commands_section and "│" in line and not line.strip().startswith("│ --"):
            ...
            if len(parts) >= 3:
                ...
                if command_part.strip():  # Only process if there's content in the command column
                    ...
                    if leading_spaces <= 10:  # Real commands have fewer leading spaces
                        command = command_part.strip().split()[0] if command_part.strip().split() else ""
                        # Only add real commands, not continuation lines from long descriptions
                        if command and command not in ["help", "Commands"] and len(command) > 1:
    ...
    for command in unique_commands:
        if command == "help": continue # 'help' command is special
