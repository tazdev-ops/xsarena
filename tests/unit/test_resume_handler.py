...
def test_resume_handler_check_and_handle_resume():
    ...
    should_create_new, existing_job_id = handler.check_and_handle_resume(
        out_path="test.txt", resume=True, overwrite=False, is_tty=False
    )
    ...
    should_create_new, existing_job_id = handler.check_and_handle_resume(
        out_path="test.txt", resume=None, overwrite=True, is_tty=False
    )
    ...
    should_create_new, existing_job_id = handler.check_and_handle_resume(
        out_path="new_test.txt", resume=None, overwrite=False, is_tty=False
    )
    assert should_create_new is True
    assert existing_job_id is None
