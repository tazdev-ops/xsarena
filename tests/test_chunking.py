...
def test_byte_chunk():
    ...
    assert len(chunks) > 0
    assert all(len(chunk.text.encode('utf-8')) <= 25 for chunk in chunks)  # Allow some buffer
...
def test_continuation_anchor():
    ...
    history = [
        ...
        MockMessage("assistant", "First response with some content that should be anchored"),
