...
try:
    from src.xsarena.core.pipeline import PipelineExecutor
except ImportError:
...
# ---------------- Mixer helpers ----------------
def _mixer_overlays():
    """Return overlay texts with safe fallbacks."""
    overlays = {
        "no_bs": (NO_BS_ADDENDUM or "Plain language. No fluff. Concrete nouns; tight sentences.").strip(),
        "narrative": (NARRATIVE_OVERLAY or "Teach-before-use narrative. Define terms at first mention.").strip(),
        "compressed": (COMPRESSED_OVERLAY or "Compressed narrative. Minimal headings; no checklists/drills; dense flow.").strip(),
        "bilingual": "If bilingual is requested, mirror structure and translate line-for-line as pairs (EN/FA).",
    }
    return overlays

def _mixer_compose_system(subject: str, base: str, chosen: list[str], extra_note: str = "") -> str:
    parts = []
    base = (base or "zero2hero").lower()
    if base == "zero2hero":
        parts.append("Goal: pedagogical manual from foundations to practice; no early wrap-ups.")
    elif base == "reference":
        parts.append("Goal: tight reference handbook; definitions first; terse, unambiguous rules and examples.")
    elif base == "pop":
        parts.append("Goal: accessible narrative explainer with vignettes; keep rigor without padding.")
    elif base == "nobs":
    ...
    for k in chosen:
        if k in ov:
            parts.append(ov[k])
    parts.append("Continuation: continue exactly from anchor; do not restart; do not summarize prematurely. If near limit, stop with: NEXT: [Continue].")
    if extra_note.strip():
        parts.append(extra_note.strip())
    if "law" in subject.lower() or "procedure" in subject.lower():
        parts.append("This is educational, not legal advice.")
    return "\n".join(parts).strip()

def _mixer_preview_path(subject: str) -> str:
    outdir = os.path.join("directives", "_mixer")
    ensure_dir(outdir)
    return os.path.join(outdir, f"{slugify(subject)}.prompt.md")

def _mixer_write_recipe(subject: str, system_text: str, base: str, out_path: str, min_chars: int, passes: int, max_chunks: int) -> str:
    ensure_dir("recipes")
    task_map = {"zero2hero": "book.zero2hero", "reference": "book.reference", "pop": "book.pop", "nobs": "book.nobs"}
    path = os.path.join("recipes", f"mixer_{slugify(subject)}.yml")
    body = [
        f"task: {task_map.get(base.lower(), 'book.zero2hero')}",
        f'subject: "{subject}"',
        "system_text: |",
        *[f"  {ln}" for ln in (system_text or "").splitlines()],
        "io:",
        '  output: file',
        f'  outPath: "{out_path}"',
        ...
        "continuation:",
        "  mode: anchor",
        f"  minChars: {int(min_chars)}",
        f"  pushPasses: {int(max(0, min(10, passes)))}",
        "  repeatWarn: true",
        "",
    ]
...
async def _mixer_edit_text(initial: str) -> str:
    """Open $EDITOR if set; otherwise allow inline paste; return final text."""
    editor = os.getenv("EDITOR") or os.getenv("VISUAL")
    if editor:
        tmp = _mixer_preview_path("edit_tmp")
        with open(tmp, "w", encoding="utf-8") as f:
            f.write(initial or "")
        try:
            subprocess.call([editor, tmp])
            return open(tmp, "r", encoding="utf-8").read().strip()
        except Exception:
            pass
    # Inline fallback
    print("(No $EDITOR). Paste edited prompt (or leave empty to keep). End with: EOF")
    edited = await read_multiline()
    return (edited.strip() or initial.strip())

async def _mixer_yes_no(prompt="Proceed? [y/N]: ") -> bool:
    print(prompt, end="", flush=True)
    loop = asyncio.get_running_loop()
    line = await loop.run_in_executor(None, sys.stdin.readline)
    return (line or "").strip().lower().startswith("y")

async def _mixer_preview_sample(subject: str, system_text: str, preview_out: str | None = None) -> str:
    """
    Generate a short, dense narrative preview (2–4 paragraphs) that reflects the final book voice.
    No outlines, no NEXT line, no headings. Returns the preview text.
    """
    SAVE_SYSTEM_STACK.append(SYSTEM_PROMPT)
    try:
        preview_capsule = (
            system_text.strip()
            + "\n\nPREVIEW MODE (for style check only):\n"
              "- Write a 2–4 paragraph representative sample in the exact voice above.\n"
              "- No outlines, no headings, no lists, no NEXT line.\n"
              "- Dense narrative; define terms inline if needed; one short vignette allowed.\n"
        )
        set_system(preview_capsule)
        user = f"Preview style sample for: {subject}\nReturn only prose paragraphs."
        print()
        ok("[mixer] Generating style preview…")
        text = await send_and_collect(build_payload(user), silent=False)
        text = (text or "").strip()
        if preview_out:
            ensure_dir(os.path.dirname(preview_out))
            with open(preview_out, "w", encoding="utf-8") as f:
                f.write(text + ("\n" if not text.endswith("\n") else ""))
            ok(f"[mixer] Preview saved → {preview_out}")
        return text
    finally:
...
# ---------------- Preview Helpers ----------------
def _preview_paths(subject: str):
    outdir = os.path.join("directives", "_preview")
    ensure_dir(outdir)
    slug = slugify(subject)
    return (
        os.path.join(outdir, f"{slug}.prompt.md"),
        os.path.join(outdir, f"{slug}.preview.md"),
        os.path.join("recipes", f"{slug}_approved.yml"),
    )

async def _preview_generate_sample(subject: str, system_text: str, preview_out: str) -> str:
    SAVE_SYSTEM_STACK.append(SYSTEM_PROMPT)
    try:
        preview_capsule = (
            system_text.strip()
            + "\n\nPREVIEW MODE:\n- Write a 2–4 paragraph representative sample in this exact voice.\n"
              "- No headings/lists. No ‘NEXT’ line."
        )
        set_system(preview_capsule)
        user = f"Preview style sample for: {subject}\nReturn only prose."
        print()
        ok("[preview] Generating style sample…")
        text = await send_and_collect(build_payload(user), silent=False)
        text = (text or "").strip()
        if preview_out:
            ensure_dir(os.path.dirname(preview_out))
            with open(preview_out, "w", encoding="utf-8") as f:
                f.write(text + ("\n" if not text.endswith("\n") else ""))
            ok(f"[preview] Sample saved → {preview_out}")
        return text
    finally:
        if SAVE_SYSTEM_STACK:
            set_system(SAVE_SYSTEM_STACK.pop())

async def _preview_before_run(subject: str, system_text: str, out_path_hint: str | None = None) -> bool:
    prompt_path, preview_path, approved_recipe = _preview_paths(subject)
    ensure_dir(os.path.dirname(prompt_path))
    ensure_dir("recipes")
    # write prompt
    with open(prompt_path, "w", encoding="utf-8") as f:
        f.write(system_text.strip() + "\n")
    ok(f"[preview] Prompt → {prompt_path}")
    # edit
    if PREVIEW_EDIT:
        edited = await _mixer_edit_text(system_text) if '_mixer_edit_text' in globals() else system_text
        system_text = edited.strip()
        with open(prompt_path, "w", encoding="utf-8") as f:
            f.write(system_text + "\n")
        ok("[preview] Prompt updated after edit.")
    # sample
    await _preview_generate_sample(subject, system_text, preview_path)
    # confirm
    if PREVIEW_AUTORUN:
        return True
    print("Start full run now? [y/N]: ", end="", flush=True)
...
# ---------------- Fast mode globals ----------------
FAST_MODE = False
FAST_SPEC = None  # dict of parsed options when --fast is used
CAPTURE_EVENT = None  # asyncio.Event created at runtime to detect capture

# ---------------- Fast mode helpers ----------------
def _compose_quick_system(subject: str, base: str, use_no_bs: bool, use_narr: bool, use_compr: bool, extra: str = "") -> str:
    parts = []
    b = (base or "zero2hero").lower()
    if b == "zero2hero":
        parts.append("Goal: pedagogical manual from foundations to practice; no early wrap-ups.")
    elif b == "reference":
        parts.append("Goal: tight reference handbook; definitions first; terse, unambiguous rules and examples.")
        ...
        parts.append("Goal: no‑bullshit manual; only what changes decisions or understanding; tight prose.")
    if use_no_bs:
        parts.append((NO_BS_ADDENDUM or "Plain language. No fluff. Concrete nouns; tight sentences.").strip())
    if use_narr:
        parts.append((NARRATIVE_OVERLAY or "Teach-before-use narrative. Define terms at first mention.").strip())
    if use_compr:
        parts.append((COMPRESSED_OVERLAY or "Compressed narrative. Minimal headings; no checklists/drills; dense flow.").strip())
    parts.append(
        "Continuation: continue exactly from anchor; do not restart; do not summarize prematurely. "
        "If near length limit, stop cleanly with: NEXT: [Continue]."
    )
    if extra.strip():
...
def _parse_fast_cli(argv: list[str]):
    """
    Minimal flag parser for --fast mode.
    Supported flags:
      --fast
      --subject "Subject"
      --base=zero2hero|reference|pop|nobs
      --no-bs / --no-no-bs
      --narrative / --no-narrative
      --compressed / --no-compressed
      --bilingual / --no-bilingual   (hint only; mixer overlay text)
      --min=INT   --passes=INT   --max=INT
      --out=PATH
      --backend=bridge|openrouter
      --capture-timeout=SECONDS
    """
    import shlex
    toks = shlex.split(" ".join(argv))
    if "--fast" not in toks:
        return False, None
    # defaults
    spec = {
    ...
    for i, tk in enumerate(toks):
        ...
        if tk == "--no-bs":
            ...
            spec["narrative"] = True
        elif tk == "--no-narrative":
            spec["narrative"] = False
        elif tk == "--compressed":
            spec["compressed"] = True
        elif tk == "--no-compressed":
            spec["compressed"] = False
        elif tk == "--bilingual":
            spec["bilingual"] = True
        elif tk == "--no-bilingual":
            spec["bilingual"] = False
        elif tk.startswith("--subject="):
            spec["subject"] = tk.split("=", 1)[1]
        elif tk == "--subject" and i + 1 < len(toks):
            spec["subject"] = toks[i + 1]
        elif tk.startswith("--base="):
            spec["base"] = tk.split("=", 1)[1].lower()
        elif tk.startswith("--min="):
            with contextlib.suppress(Exception): spec["min"] = int(tk.split("=", 1)[1])
        elif tk.startswith("--passes="):
            with contextlib.suppress(Exception): spec["passes"] = int(tk.split("=", 1)[1])
        elif tk.startswith("--max="):
            with contextlib.suppress(Exception): spec["max"] = int(tk.split("=", 1)[1])
        elif tk.startswith("--out="):
            spec["out"] = tk.split("=", 1)[1]
        elif tk.startswith("--backend="):
            spec["backend"] = tk.split("=", 1)[1].lower()
        elif tk.startswith("--capture-timeout="):
            with contextlib.suppress(Exception): spec["capture_timeout"] = int(tk.split("=", 1)[1])
    if not spec["subject"]:
        err("Fast mode needs --subject \"...\"")
        raise SystemExit(2)
    return True, spec

async def _wait_for_capture(timeout_sec: int = 180) -> bool:
    """Wait until SESSION_ID and MESSAGE_ID arrive via userscript capture."""
    ...
    try:
        await asyncio.wait_for(CAPTURE_EVENT.wait(), timeout=timeout_sec)
        return True
    except asyncio.TimeoutError:
        return False

async def _fast_mode_entry(spec: dict):
    """Run non-interactively: wait for capture (bridge) then start the book."""
    global BACKEND, SYSTEM_PROMPT, OUTPUT_MIN_CHARS, OUTPUT_PUSH_MAX_PASSES
    global CONT_MODE, AUTO_ON, AUTO_OUT, AUTO_MAX, AUTO_TASK, AUTO_COUNT, LAST_NEXT_HINT
    global OUTPUT_BUDGET_SNIPPET_ON, COVERAGE_HAMMER_ON

    subj = spec["subject"]
    base = spec["base"]
    use_no_bs = spec["no_bs"]
    use_narr = spec["narrative"]
    use_compr = spec["compressed"]
    backend = spec["backend"]
    outp = spec["out"] or next_available_path(os.path.join("books", f"{slugify(subj)}.final.md"))
    minc = int(spec["min"])
    passes = int(spec["passes"])
    maxc = int(spec["max"])

    BACKEND = backend if backend in ("bridge", "openrouter") else "bridge"
    ok(f"[fast] Backend: {BACKEND}")

    # Compose system prompt
    extra = ""
    if "law" in subj.lower():
        extra = "Explain doctrine by what it lets actors do and forbids; name the trade-offs. No slogans."
    SYSTEM_PROMPT = _compose_quick_system(subj, base, use_no_bs, use_narr, use_compr, extra)

    # Knobs
    CONT_MODE = "anchor"
    OUTPUT_MIN_CHARS = max(1000, minc)
    OUTPUT_PUSH_MAX_PASSES = max(0, min(10, passes))
    COVERAGE_HAMMER_ON = True
    OUTPUT_BUDGET_SNIPPET_ON = True

    if BACKEND == "bridge":
        # Instruct user to open the site and click Retry once
        print()
        ok("[fast] Waiting for capture from browser (Tampermonkey userscript).")
        print("  1) Open https://lmarena.ai (with the userscript enabled)")
        print("  2) Click 'Retry' once on any message")
        print(f"  3) We will wait up to {spec['capture_timeout']}s for capture to complete…")
        # Signal browser to capture once
        globals()["CAPTURE_FLAG"] = True
        captured = await _wait_for_capture(timeout_sec=int(spec["capture_timeout"]))
        if not captured:
            err("[fast] Capture timed out. Exiting.")
            return
        ok("[fast] Capture complete. Starting run…")
    else:
        ok("[fast] OpenRouter mode — no capture needed.")
    ...
    AUTO_COUNT = 0
    LAST_NEXT_HINT = None
    ok(f"[fast] Autopilot ON → {AUTO_OUT} (max_chunks={AUTO_MAX}, minchars={OUTPUT_MIN_CHARS}, passes={OUTPUT_PUSH_MAX_PASSES})")
    if globals().get("AUTO_TASK") and not AUTO_TASK.done():
        AUTO_TASK.cancel()
    globals()["AUTO_TASK"] = asyncio.create_task(autorun_loop())
    await AUTO_TASK
    ok("[fast] Run finished.")
...
async def id_capture_update(request: web.Request):
    ...
    try:
        ...
        print("> ", end="", flush=True)
        # Signal fast-mode waiter if present
        try:
            if globals().get("CAPTURE_EVENT"):
                globals()["CAPTURE_EVENT"].set()
        except Exception:
            pass
        return _add_cors(web.json_response({"status": "ok"}))
    except Exception as e:
...
async def _send_and_collect_openrouter(payload: dict, silent: bool = False) -> str:
    ...
    try:
        for chunk in stream:
            ...
            if CANCEL_REQUESTED:
                if not silent:
                    print(f"\n{C.WARN}! Stream cancelled by user request{C.R}")
...
def _load_yaml_or_json(path: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            raise ValueError(f"Could not parse {path} as either YAML or JSON: {str(e)}")
...
async def run_recipe_file(path: str):
    try:
        ...
        err(f"Error loading recipe: {str(e)}")
...
async def run_recipe(rec: dict):
    ...
    if rec.get("system_text") is not None:
        # Use custom system text if provided (e.g., from Mixer)
        sys_text = rec["system_text"]
    elif task in (
        "book.zero2hero",
        "book.reference",
        "book.pop",
    ...
    # apply overlays (only if not using custom system_text)
    if rec.get("system_text") is None:
    ...
    if PREVIEW_ON:
        ...
        if not cont:
            warn("[preview] Aborted before full run.")
            if SAVE_SYSTEM_STACK:
                set_system(SAVE_SYSTEM_STACK.pop())
            return
...
def help_text():
    ...
    print("  /z2h <subject> [--plan] [--max=N] [--window=N] [--outdir=DIR]      (alias for /book.nobs)")
    ...
    print("Mixer:")
    print('  /mix.list                      Show base modes and overlays')
    print('  /mix.start "Subject" [--base=zero2hero|reference|pop|nobs] [--no-bs|--no-no-bs] [--narrative|--no-narrative]')
    print('                                [--compressed|--no-compressed] [--bilingual] [--min=N] [--passes=N] [--max=N]')
    print('                                [--out=./books/file.md] [--no-edit] [--dry]')
    print('  /mix.preview "Subject" [...]   Compose + edit → generate short STYLE PREVIEW → optional full run')
    print("Preview:")
    print("  /preview.on | /preview.off      Toggle universal preview-before-start")
    print("  /preview.edit on|off            Open $EDITOR to edit prompt before sampling")
    print("  /preview.autorun on|off         Auto-continue after preview without asking")
...
async def _handle_command(line):
    ...
    if line.startswith("/"):
        ...
        # Prompt Booster commands
            ...
            globals()["PREVIEW_ON"] = True
            ok("Preview-before-start: ON")
            return
        elif cmd == "/preview.off":
            globals()["PREVIEW_ON"] = False
            ok("Preview-before-start: OFF")
            return
        elif cmd == "/preview.edit":
            if len(parts) >= 2 and parts[1].lower() in ("on", "off"):
                globals()["PREVIEW_EDIT"] = (parts[1].lower() == "on")
                ok(f"Preview edit: {PREVIEW_EDIT}")
            else:
                err("Usage: /preview.edit on|off")
            return
        elif cmd == "/preview.autorun":
            if len(parts) >= 2 and parts[1].lower() in ("on", "off"):
                globals()["PREVIEW_AUTORUN"] = (parts[1].lower() == "on")
                ok(f"Preview autorun: {PREVIEW_AUTORUN}")
            else:
                err("Usage: /preview.autorun on|off")
            return
        elif cmd == "/quickpaste":
        ...
        # Recipe runner commands
            ...
            try:
                ...
                err(f"Error running recipe: {str(e)}")
                prompt()
                return
        ...
        # ---------------- Mixer commands ----------------
            ...
            print("Base modes:\n  - zero2hero\n  - reference\n  - pop\n  - nobs")
            print("\nOverlays:")
            for k in ("no_bs", "narrative", "compressed", "bilingual"):
                if k in ov:
                    print(f"  - {k}: {ov[k].splitlines()[0][:80]}…")
            print(hr())
            return
            ...
            passes = 1
            max_chunks = 12
            out_path = f"./books/{slugify(subject)}.final.md"
            do_edit = True
            dry = False
            # Parse flags (simple split; allow quotes via shlex)
            tokens = shlex.split(line)
            for tk in tokens:
                if tk.startswith("--base="):
                    base = tk.split("=", 1)[1].strip().lower()
                elif tk == "--no-bs":
                    use_no_bs = True
                elif tk == "--no-no-bs":
                    use_no_bs = False
                    ...
                    with contextlib.suppress(Exception): min_chars = int(tk.split("=",1)[1])
                elif tk.startswith("--passes="):
                    with contextlib.suppress(Exception): passes = int(tk.split("=",1)[1])
                elif tk.startswith("--max="):
                    with contextlib.suppress(Exception): max_chunks = int(tk.split("=",1)[1])
                elif tk.startswith("--out="):
                    out_path = tk.split("=",1)[1]
                elif tk == "--no-edit":
                    do_edit = False
                elif tk == "--dry":
                    dry = True
            chosen = []
            if use_no_bs: chosen.append("no_bs")
            if use_narr: chosen.append("narrative")
            if use_compr: chosen.append("compressed")
            if use_bi: chosen.append("bilingual")
            extra = ""
            if "law" in subject.lower():
                extra = "Explain doctrine by what it lets actors do and forbids; name the trade-offs. No slogans or keyword stuffing."
            final_system = _mixer_compose_system(subject, base, chosen, extra)
            # Save preview
            pv_path = _mixer_preview_path(subject)
            with open(pv_path, "w", encoding="utf-8") as f:
                f.write(final_system)
            ok(f"[mixer] Prompt preview → {pv_path}")
            # Optional edit
            if do_edit:
                final_system = await _mixer_edit_text(final_system)
                with open(pv_path, "w", encoding="utf-8") as f:
                    f.write(final_system)
                ok("[mixer] Prompt updated after edit.")
            # Write portable recipe
            recipe_path = _mixer_write_recipe(subject, final_system, base, out_path, min_chars, passes, max_chunks)
            ok(f"[mixer] Recipe → {recipe_path}")
            if dry:
                warn("[mixer] Dry-run: not starting. You can run the recipe later.")
                return
            # Build and run via existing recipe runner
            rec = {
                "task": {"zero2hero":"book.zero2hero","reference":"book.reference","pop":"book.pop","nobs":"book.nobs"}.get(base,"book.zero2hero"),
                "subject": subject,
                "system_text": final_system,
                "hammer": True,
                "continuation": {"mode":"anchor","minChars": int(min_chars), "pushPasses": int(max(0,min(10,passes))), "repeatWarn": True},
            ...
            if not m:
                err('Usage: /mix.preview "Subject" [--base=zero2hero|reference|pop|nobs] '
                    '[--no-bs|--no-no-bs] [--narrative|--no-narrative] [--compressed|--no-compressed] '
                    '[--bilingual|--no-bilingual] [--min=N] [--passes=N] [--max=N] [--out=PATH] '
                    '[--no-edit] [--autorun] [--norun]')
                return
            subject = m[0]
            # Defaults (preview uses same knobs you'll later run with)
            base = "zero2hero"
            use_no_bs = True
            use_narr = False
            use_compr = True
            ...
            out_path = f"./books/{slugify(subject)}.final.md"
            do_edit = True
            autorun = False   # if true, skip confirm and start full run after preview
            norun = False     # if true, never start full run (preview only)
            tokens = shlex.split(line)
            for tk in tokens:
                if tk.startswith("--base="):
                    base = tk.split("=", 1)[1].strip().lower()
            ...
            # Save and (optionally) edit the composed prompt
            pv_path = _mixer_preview_path(subject)
            with open(pv_path, "w", encoding="utf-8") as f:
                f.write(final_system)
            ok(f"[mixer] Prompt preview → {pv_path}")
            ...
            # Generate style preview (short dense narrative)
            prev_file = pv_path.replace(".prompt.md", ".preview.md")
            await _mixer_preview_sample(subject, final_system, prev_file)

            if norun:
                warn("[mixer] Preview-only requested (--norun). Not starting full job.")
                return
            if not autorun:
                proceed = await _mixer_yes_no("Looks good. Start full run now? [y/N]: ")
                if not proceed:
                    warn("[mixer] Aborted before full run.")
                    return

            # Build recipe and start full job
            recipe_path = _mixer_write_recipe(subject, final_system, base, out_path, min_chars, passes, max_chunks)
            ok(f"[mixer] Recipe → {recipe_path}")
            rec = {
                "task": {"zero2hero":"book.zero2hero","reference":"book.reference","pop":"book.pop","nobs":"book.nobs"}.get(base,"book.zero2hero"),
        ...
        if line == "/run.inline":
            ...
            if rec_text:
                ...
                try:
                    ...
                    err(f"Error running inline recipe: {str(e)}")
            continue
...
async def repl():
    ...
    while True:
        ...
        if line.startswith("/"):
            ...
            elif cmd == "/z2h":  # Alias for /book.nobs to match QuickPaste specification
                if len(parts) < 2:
                    err(
                        "Usage: /z2h <subject> [--plan] [--max=N] [--window=N] [--outdir=DIR]"
                    )
                    prompt()
                    continue
                tokens = line.split()[1:]
            ...
            # Prompt Booster commands
                ...
                if len(parts) >= 2 and parts[1].strip().lower() in ("next", "system"):
                    where = parts[1].strip().lower()
                prompt_apply(where)
            # Mixer: delegate to main handler to avoid duplication
            elif cmd in ("/mix.list", "/mix.start", "/mix.preview"):
                await _handle_command(line)
                prompt()
                continue
            elif cmd == "/quickpaste":
...
def main():
    _load_macros()
    # Parse --fast flags early
    global FAST_MODE, FAST_SPEC
    try:
        FAST_MODE, FAST_SPEC = _parse_fast_cli(sys.argv[1:])
    except SystemExit:
        return
    app = web.Application()
    ...
    async def _run():
        ...
        # Fast mode: run non-interactively; else fall back to REPL
        if FAST_MODE and FAST_SPEC:
            await _fast_mode_entry(FAST_SPEC)
        else:
            await repl()
