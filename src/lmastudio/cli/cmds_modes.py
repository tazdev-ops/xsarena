...
@app.command("tavern")
def set_tavern_mode(
    enabled: bool = typer.Argument(..., help="Enable or disable tavern mode (True/False)")
):
...
@app.command("bypass")
def set_bypass_mode(
    enabled: bool = typer.Argument(..., help="Enable or disable bypass mode (True/False)")
):
...
def set_image_handling(
    enabled: bool = typer.Argument(..., help="Enable or disable image handling (True/False)")
):
...
def update_available_models():
    ...
    typer.echo("Model update endpoint ready. The userscript can POST page HTML to /internal/update_available_models to update available_models.json")
