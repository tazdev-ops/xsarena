...
class PromptCompositionLayer:
    ...
    def _load_extended_templates(self):
        ...
        try:
            from .templates import NARRATIVE_OVERLAY, COMPRESSED_OVERLAY, NO_BS_ADDENDUM
        ...
        try:
            ...
            if hasattr(legacy, "NARRATIVE_OVERLAY") and legacy.NARRATIVE_OVERLAY:
                self.OVERLAYS["narrative"] = getattr(legacy, "NARRATIVE_OVERLAY").strip()
            if hasattr(legacy, "COMPRESSED_OVERLAY") and legacy.COMPRESSED_OVERLAY:
                self.OVERLAYS["compressed"] = getattr(legacy, "COMPRESSED_OVERLAY").strip()
            if hasattr(legacy, "NO_BS_ADDENDUM") and legacy.NO_BS_ADDENDUM:
                self.OVERLAYS["no_bs"] = getattr(legacy, "NO_BS_ADDENDUM").strip()
        except ImportError:
            pass
    ...
    def compose(
        ...
        # Base intent
        if base == "zero2hero":
            parts.append("Goal: pedagogical manual from foundations to practice with steady depth; no early wrap-ups.")
        elif base == "reference":
    ...
    def lint(self, subject: str, base: str, overlays: List[str]) -> List[str]:
        ...
        if len(subject.strip()) < 3:
            warnings.append("Subject is very short (< 3 chars), consider being more specific")
        
        # Check for common overlay conflicts
        if "narrative" in overlays and "compressed" in overlays:
            warnings.append("Using both 'narrative' and 'compressed' overlays may create conflicting styles")
        
        # Check base mode appropriateness for subject
        if "law" in subject.lower() and base == "pop":
            warnings.append("For legal subjects, 'reference' or 'nobs' base modes might be more appropriate than 'pop'")
