...
class Job:
    ...
    artifacts: Dict[str, str] = field(default_factory=dict)  # "outline": path, "plan": path, ...
...
class JobRunner:
    ...
    def _count_running_by_backend(self, backend: str) -> int:
        ...
        for job in current_jobs:
            if job.id in self.running_jobs and job.backend == backend:
                count += 1
        return count
    ...
    def submit(self, playbook: Dict[str, Any], params: Dict[str, Any]) -> str:
        ...
        job = Job(id=job_id, name=playbook.get("name", "unnamed"), playbook=playbook, params=params, backend=backend)
    ...
    async def run_job_with_scheduler(self, job_id: str) -> None:
        ...
        while not self.can_run_now(job_id, job.backend):
            ...
            self.write_event(
                job_id, {"type": "waiting_for_scheduler", "backend": job.backend, "reason": "scheduler_constraints"}
            )
    ...
    async def _execute_job_async(self, job_id: str) -> None:
        ...
        try:
            ...
            state.output_min_chars = job.params.get("continuation", {}).get("minChars", 3000)
            state.watchdog_secs = job.playbook.get("failover", {}).get("watchdog_secs", 45)
            ...
            if job.playbook.get("outline_first", False):
                ...
                self.write_event(job_id, {"type": "outline_completed", "chars": len(outline_response)})
            ...
            if stages:
                ...
                executor = Z2HExecutor(self, job_id, job.playbook, job.params, job.backend)
                ...
                if result["status"] == "DONE":
                    ...
                    job.artifacts.update({f"aid_{i}": path for i, path in enumerate(result.get("aids_paths", []))})
                elif result["status"] == "FAILED":
                    job.state = "FAILED"

            self._update_job(job)
            self.write_event(job_id, {"type": "job_completed" if job.state == "DONE" else "job_ended"})
        ...
        for stage in job.playbook.get("stages", []):
            ...
            try:
                ...
                if agent:
                    ...
                    self.write_event(
                        job_id, {"type": "stage_failed", "stage": stage, "error": f"Unknown agent: {stage}"}
                    )
                    job.state = "FAILED"
                    break

            except Exception as e:
                self.write_event(job_id, {"type": "stage_failed", "stage": stage, "error": str(e)})
                job.state = "FAILED"
                break
    ...
    def _create_transplant_summary(self, job_id: str, section_num: int = None, subject: str = None) -> str:
    ...
    def set_budget(self, job_id: str, usd: float):
        ...
        self.write_event(job_id, {"type": "budget_set", "value": usd, "message": f"Budget set to ${usd:.2f} USD"})
    ...
    def fork(self, job_id: str, backend: str = "openrouter"):
        ...
        new_job = Job(
            id=new_job_id,
            name=f"Fork of {original_job.name}" if original_job.name else f"Fork of {job_id}",
        ...
        if transplant_summary:
            ...
            if "system_text" in new_job.playbook:
                new_job.playbook["system_text"] = transplant_summary + "\n\n" + new_job.playbook["system_text"]
            elif "system_text" in new_job.params:
                new_job.params["system_text"] = transplant_summary + "\n\n" + new_job.params["system_text"]
        ...
        self.write_event(job_id, {"type": "job_forked_out", "new_job_id": new_job_id, "backend": backend})
    ...
    def run_aids(self, job: Job):
        ...
        for aid_type in aids:
            if aid_type == "cram":
                ...
                self.write_event(job.id, {"type": "aid_generated", "aid": "cram", "path": cram_path})
                ...
                self.write_event(job.id, {"type": "aid_generated", "aid": "flashcards", "path": flashcards_path})
                ...
                self.write_event(job.id, {"type": "aid_generated", "aid": "glossary", "path": glossary_path})

            elif aid_type == "index":
                index_content = f"# Index for {subj}\n\n## Topics Covered\n\n- Topic 1: Page X\n- Topic 2: Page Y\n- Topic 3: Page Z"
                index_path = write_aid(job.id, "index", index_content)
                job.artifacts["index"] = index_path
                self.write_event(job.id, {"type": "aid_generated", "aid": "index", "path": index_path})
                ...
                self.write_event(
                    job.id,
                    {"type": "aid_requested", "aid": "audio", "message": "audio generation to be run after publishing"},
                )
