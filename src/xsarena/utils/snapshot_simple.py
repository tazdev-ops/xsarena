...
def _apply_excludes(candidates: Set[Path], exclude_patterns: List[str], reincludes: List[str]) -> Set[Path]:
...
def collect_paths(mode: str, include_git_tracked: bool = False, include_untracked: bool = False) -> List[Path]:
    """Collect paths based on mode and git options."""
    cfg = read_snapshot_config()
    
    if include_git_tracked:
        return collect_git_files(include_untracked, cfg.get("modes", {}).get(mode, {}).get("exclude", []))
...
def collect_git_files(include_untracked: bool, exclude_patterns: List[str]) -> List[Path]:
...
def build_jobs_summary() -> str:
    ...
    for job_dir in job_dirs[:10]:  # Top 10 recent jobs
        ...
        try:
            ...
            event_counts = {"chunk_done": 0, "retry": 0, "error": 0, "watchdog": 0, "failover": 0}
...
def build_manifest(files: List[Path]) -> str:
    ...
    for file_path in files:
        try:
            ...
            size = len(content)
            manifest.append(f"  {digest[:12]}  {size:>8} bytes  {file_path.relative_to(ROOT)}")
        except Exception:
            manifest.append(f"  {'[ERROR]':<12}  {'ERROR':>8} bytes  {file_path.relative_to(ROOT)}")
...
def write_pro_snapshot(
    ...
    # Write the output
    output_path = Path(out_path) if out_path else Path("~/xsa_snapshot_pro.txt").expanduser()
...
def get_review_artifacts() -> str:
    ...
    for item in review_dir.iterdir():
        if item.is_file():
            try:
                content, truncated = safe_read_text(item, 5000)  # Limit to first 5000 chars
...
def render_directory_tree(path: Path, prefix: str = "", max_depth: int = 3, current_depth: int = 0) -> str:
        ...
        for i, item in enumerate(items):
            ...
            if item.is_dir():
                if current_depth < max_depth:
                    ...
                    tree_lines.append(render_directory_tree(item, prefix + extension, max_depth, current_depth + 1))
            else:
                tree_lines.append(f"{prefix}{extension}{connector}{item.name}")
