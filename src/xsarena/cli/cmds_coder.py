...
app = typer.Typer(help="Advanced coding session with tickets, patches, and git integration")


@app.command("start")
def coder_start(root: str = typer.Option(".", "--root", "-r", help="Project root directory")):
...
def ticket_new(
    ...
    lines: Optional[str] = typer.Option(None, "--lines", "-l", help="Line numbers (e.g., '12,19' or '15-20')"),
...
def ticket_next():
    ...
    if ticket:
        typer.echo(f'{{"id":"{ticket.id}","file":"{ticket.file}","lines":"{ticket.lines}","note":"{ticket.note}"}}')
    else:
...
def patch_command(
    ...
    if action == "dry":
        ...
        if result["error"]:
            ...
            typer.echo(f"[coder] Dry run OK, would apply {result['applied_hunks']} hunks")
    elif action == "apply":
...
@app.command("branch")
def branch_command(ticket_id: str = typer.Argument(..., help="Ticket ID for branch name")):
...
def test_skeleton(module_path: str = typer.Argument(..., help="Module path to create test for")):
...
def import_ruff_tickets():
    ...
    for ticket_file in tickets_dir.glob("*.json"):
        try:
            ...
            session = CoderSession(".")
            ticket_id = session.ticket_new(file=file_path, lines=str(line), note=f"Ruff: {code} - {message}")

            created_count += 1
            typer.echo(f"[coder] Created ticket {ticket_id} from ruff ticket: {file_path}:{line} - {code}")
