...
def _latest_job_id() -> Optional[str]:
    ...
    if not jobs.exists(): return None
...
def _summarize_events(job_dir: Path) -> dict:
    ...
    out = {"chunks": 0, "retries": 0, "failovers": 0, "watchdogs": 0, "first_ts": "", "last_ts": ""}
    if not evp.exists(): return out
    first = last = ""
    for line in evp.read_text(encoding="utf-8", errors="ignore").splitlines():
        ...
        if ts: last = ts
        if t == "chunk_done": out["chunks"] += 1
        elif t == "retry": out["retries"] += 1
        elif t == "failover": out["failovers"] += 1
        elif t == "watchdog_timeout": out["watchdogs"] += 1
    out["first_ts"], out["last_ts"] = first, last
    return out
def _pack(folder: Path, dest: Path):
    dest.parent.mkdir(parents=True, exist_ok=True)
    with tarfile.open(dest, "w:gz") as tar: tar.add(folder, arcname=folder.name)
def _header(kind: str) -> str:
    return f"# XSArena Report ({kind})\n\nGenerated: {time.strftime('%Y-%m-%d %H:%M:%S')}\nNote: Redacted bundle.\n\n" \
           "## Include in your message\n- Command used\n- Expected vs actual\n- Attach this tar.gz\n"

def _common(root: Path, cli: CLIContext, job_id: Optional[str], book_path: Optional[str]):
    _write(root / "report.md", _header("bundle"))
    cfg = Path(".xsarena/config.yml"); sst = Path(".xsarena/session_state.json")
    if cfg.exists(): _write(root / "config.yml", redact(_read(cfg)))
    if sst.exists(): _write(root / "session_state.json", redact(_read(sst)))
    for f in ["pyproject.toml", "README.md"]:
        fp = Path(f); if_exists = fp.exists()
        if if_exists: _write(root / f, redact(_read(fp)))
    rules = Path("directives/_rules/rules.merged.md")
    if rules.exists(): _write(root / "rules.merged.md", redact(_read(rules)))
    if book_path:
        bp = Path(book_path)
        if bp.exists(): _write(root / "book_sample.md", _read(bp, 300_000))
    if job_id:
        ...
        if jd.exists():
            ...
            _write(root / "job_summary.json", json.dumps(_summarize_events(jd), indent=2))

@app.command("quick")
def report_quick(
    ctx: typer.Context,
    book: Optional[str] = typer.Option(None, "--book", help="Attach head/tail sample from this book file")
):
    cli: CLIContext = ctx.obj
    rid = _ts(); outdir = Path("review") / f"report_{rid}"; outdir.mkdir(parents=True, exist_ok=True)
    _common(outdir, cli, _latest_job_id(), book)
    tar = Path("review") / f"report_{rid}.tar.gz"; _pack(outdir, tar); typer.echo(f"[report] quick → {tar}")
...
def report_job(
    ...
    tar = Path("review") / f"report_{rid}.tar.gz"; _pack(outdir, tar); typer.echo(f"[report] job {job_id} → {tar}")
...
def report_full(
    ...
    if include_recipes:
        for p in Path("recipes").rglob("*.yml"):
            try: _write(outdir / "recipes" / p.name, redact(_read(p, 120_000)))
            except Exception: pass
    if include_directives:
        for p in Path("directives").rglob("*"):
            if p.is_file() and p.suffix in {".md", ".txt"} and p.stat().st_size < 200_000:
