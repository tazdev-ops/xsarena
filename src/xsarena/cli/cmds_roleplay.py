...
def rp_start(
    ...
    s = new_session(
        name=name, persona=persona, overlay=overlay, backend=backend, model=model, rating=rating, safeword=safeword
    )
    print(
        json.dumps(
            {"rp_id": s.id, "name": s.name, "persona": persona, "backend": backend, "model": model or "(default)"}
        )
    )
...
def rp_say(sess_id: str, text: str):
    ...
    if s.boundaries.safeword and s.boundaries.safeword in text:
        append_turn(sess_id, "assistant", "Safeword received. Pausing. Do you want a summary or to resume?")
        print("PAUSED.")
        return
    # Append user turn
    ...
    user = (
        mem
        + "\n".join(f"{t['role']}: {t['content']}" for t in turns if t["role"] in ("user", "assistant"))
        + "\nassistant:"
    )
...
@app.command("mem")
def rp_memory(
    sess_id: str, add: Optional[str] = typer.Option(None, "--add"), show: bool = typer.Option(False, "--show")
):
    s = load_session(sess_id)
    if add:
        s.memory.append(add)
...
def rp_bounds(
    ...
    save_session(s)
    print(json.dumps({"rating": s.boundaries.rating, "safeword": s.boundaries.safeword}))
