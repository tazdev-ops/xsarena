...
async def _seed_and_continue(
    cli: CLIContext,
    book_path: Path,
    system_text: str,
    init_chunks: Optional[int],
    min_chars: int,
    passes: int,
):
    st = cli.state
    ...
    seed_user = (
        (await cli.engine.build_anchor_continue_prompt(anchor))
        if anchor
        else "Continue."
    )
    ...
    if init_chunks is None:
        await cli.engine.autopilot_run(
            initial_prompt="BEGIN", max_chunks=None
        )  # until model emits NEXT: [END]
    else:
...
def continue_start(
    ...
    book_file: str = typer.Argument(
        ...,
        help="Existing book file to continue (e.g., ./books/finals/subject.final.md)",
    ),
    subject: Optional[str] = typer.Option(
        None, "--subject", "-s", help="Subject title (if different from filename)"
    ),
    length: str = typer.Option(
        "long", "--length", help="Per-message length: standard|long|very-long|max"
    ),
    span: str = typer.Option(
        "book", "--span", help="Total span: medium|long|book (ignored if --until-end)"
    ),
    ...
    extra_file: List[str] = typer.Option(
        ...
        "--extra-file",
        "-E",
        help="Append file content(s) to system prompt (e.g., directives/_rules/rules.merged.md)",
    ),
    until_end: bool = typer.Option(
        False,
        "--until-end",
        help="Ignore span and continue until the model emits NEXT: [END]",
    ),
    ...
    async def _run():
        await _seed_and_continue(
            cli,
            p,
            system_text,
            init_chunks=total_chunks,
            min_chars=L["min"],
            passes=L["passes"],
        )
