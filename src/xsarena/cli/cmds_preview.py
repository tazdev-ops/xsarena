...
def preview_run(
    ...
    sample: bool = typer.Option(False, "--sample/--no-sample", help="Generate a real 2-4 paragraph sample using the engine")
):
    ...
    if sample:
        ...
        try:
            sample_text = asyncio.run(cli.engine.send_and_collect(sample_prompt, system_prompt=system_text))
    ...
    if autorun:
        ...
        playbook = {
            ...
            "hammer": True,
            "outline_first": True,
            "failover": {"watchdog_secs": 90, "max_retries": 3, "fallback_backend": "openrouter"},
        }
