...
def _parse_jsonc(jsonc_string: str) -> dict:
    lines = [line for line in jsonc_string.splitlines() if not line.strip().startswith('//')]
    return json.loads("\n".join(lines))
...
async def convert_openai_to_lmarena_payload(openai_data: dict, session_id: str, message_id: str) -> dict:
    ...
    if CONFIG.get("tavern_mode_enabled"):
        ...
        messages = ([{"role": "system", "content": system_prompts}] if system_prompts else []) + other_messages
    ...
    for msg in message_templates:
        if msg['role'] == 'system':
            msg['participantPosition'] = 'b' if mode == 'direct_chat' else target_participant
        else:
            msg['participantPosition'] = 'a' if mode == 'direct_chat' else target_participant
    ...
    try:
        while True:
            ...
            if request_id in response_channels:
                ...
                logger.warning(f"Received data for unknown or closed request_id: {request_id}")
...
async def chat_completions(request: Request):
    ...
    if not session_id or not message_id:
        raise HTTPException(status_code=400, detail="Session ID/Message ID not configured.")
    
    request_id = str(uuid.uuid4())
    response_channels[request_id] = asyncio.Queue()
    
    try:
        lmarena_payload = await convert_openai_to_lmarena_payload(openai_req, session_id, message_id)
        await browser_ws.send_json({"request_id": request_id, "payload": lmarena_payload})
...
@app.get("/health")
async def health():
    return JSONResponse({"status": "ok", "ts": time.strftime("%Y-%m-%dT%H:%M:%S"), "ws_connected": browser_ws is not None})
